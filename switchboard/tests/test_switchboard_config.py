...
def test_every_playback_prompt_is_actually_shipped() -> None:
    """A Playback of a missing sound file is a SILENT no-op.

    Asterisk logs a warning and carries on, so the caller simply hears nothing
    where the explanation was supposed to be — a feature that is wired, tested
    against the generated config, and inert in production. That is the exact
    pattern this project keeps hitting, and a generated dialplan is the easiest
    place to introduce it: the string is right there in the file and nothing
    checks that the bytes exist.

    Every `switchboard/...` prompt referenced by any generated context must exist
    as a shipped .wav."""
    rooms = sbc.valid_rooms([{"ext": "11", "name": "Kitchen", "secret": "s1"},
                             {"ext": "12", "name": "Office", "secret": "s2"}])
    sounds = (Path(__file__).resolve().parents[1] / "rootfs" / "var" / "lib"
              / "asterisk" / "sounds" / "en" / "switchboard")
    seen = set()
    for trunk in ({"enabled": False},
                  {"enabled": True, "provider_host": "example.net", "username": "u",
                   "secret": "x", "dial_prefix": "9", "outbound_caller_id": "5555550100"},
                  {"enabled": True, "provider_host": "example.net", "username": "u",
                   "secret": "x", "direct_dial": True, "outbound_caller_id": "5555550100"}):
        e = sbc.render_extensions({"rooms": rooms, "trunk": trunk,
                                   "operator": {"enabled": True}})
        seen |= set(re.findall(r"Playback\(switchboard/([a-z0-9-]+)\)", e))
        seen |= set(re.findall(r"Background\(switchboard/([a-z0-9-]+)\)", e))
    check(f"the generated dialplan references prompts at all ({len(seen)} found)",
          len(seen) >= 3)
    missing = sorted(n for n in seen if not (sounds / f"{n}.wav").exists())
    check(f"every referenced prompt is shipped as a .wav (missing: {missing})",
          not missing)


def test_generated_files_are_dated() -> None:
    """A captured config must be distinguishable from the one that ran.

    An audit pulled extensions.conf out of the container and reasoned about six
    days of logs against it, with no way to tell the file had been regenerated
    twice inside that window. A date on line one makes "is this the dialplan that
    produced these logs?" answerable rather than assumed."""
    rooms = sbc.valid_rooms([{"ext": "11", "name": "Kitchen", "secret": "s1"}])
    opts = {"rooms": rooms, "trunk": {"enabled": False}, "operator": {"enabled": True}}
    stamp = re.compile(r"^;; GENERATED by switchboard-config at "
                       r"\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}[^—]*— edits here are overwritten\.$")
    checked = 0
    for name, text in (("extensions.conf", sbc.render_extensions(opts)),
                       ("pjsip.conf", sbc.render_pjsip(opts))):
        first = text.strip().splitlines()[0]
        check(f"{name}: opens with a DATED generation banner (got {first[:70]!r})",
