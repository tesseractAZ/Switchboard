...
- **Home Assistant host:** `192.168.1.152`, mask **/22** (subnet
  `192.168.1.0`–`192.168.1.255`). Asterisk listens here on **UDP 5060**
  (add-on uses host networking).
- **Grandstream GXW4216 V2:** `192.168.1.65` — **inside the same /22**, so HA
...
1. **Firmware-update the GXW4216 V2** (before provisioning, so we don't redo it):
   - Web UI: `http://192.168.1.65`, user `admin`, password on the unit's sticker
...
3. **Provision the gateway** (DOCS §4):
   - Profile 1 → SIP Server `192.168.1.152`, transport UDP, NAT Traversal No.
...
```bash
ping 192.168.1.65
curl -s http://192.168.1.65/        # login page
